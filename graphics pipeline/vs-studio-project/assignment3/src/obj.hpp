...
void LoadTexture() {
	...
	TextureArr = (mVec3*)malloc(TW* TH * sizeof(mVec3));
	for (int r = 0; r < TH; r++)
		for (int c = 0; c < TW; c++) {
			TextureArr[r * TW + c].x = float(TextureImg(c, r, 0)) / 255.0f;
			TextureArr[r * TW + c].y = float(TextureImg(c, r, 1)) / 255.0f;
			TextureArr[r *TW + c].z = float(TextureImg(c, r, 2)) / 255.0f;
		}
}
mVec3 LookupTexel(mVec2<float> st) {
...
public:
	vertex a, b, c;
	inline TriangleWithAttributes(vertex a, vertex b, vertex c) :a(a), b(b), c(c) {
	}
...
private:
	...
	inline void VertexesProcess(RenderObject yu);
	inline void Rasterization(ShadingMaterial myu);
	inline void FragmentProcess(mVec3* Fb);
	mVec3 LightSource;
...
public:
	...
	inline void Render(RenderObject yu, mVec3* Fb);
	inline void GraphicsPipeline::clearPipeline();
};

inline void GraphicsPipeline::Render(RenderObject yu, mVec3* Fb) {
...
inline void GraphicsPipeline::clearPipeline() {
	...
	TargetRenderVertexes.clear();
	FragmentsAfterRasterization.clear();
	//	FragmentsAfterRasterization.swap(vector<fragment>());
}

inline void GraphicsPipeline::VertexesProcess(RenderObject yu) {
	...
	for (mVec3& Vertex : yu.Vertexes) {
	   ...
	   //-------------------------------
		...
		vertex tmp = { HomoCoordinates.tomVec3(), yu.VertexesNormal[Vid],shading, EyeHomoCoordinates.tomVec3(),yu.VertexesTex[Vid]};
...
inline void  GraphicsPipeline::Rasterization(ShadingMaterial myu) {
...
inline void GraphicsPipeline::FragmentProcess(mVec3* Fb) {
	...
	for (fragment TriFrag : FragmentsAfterRasterization) {
		...
		if (depthbuffer[ScreenXY.y*w + ScreenXY.x] > distance ) {
			...
			Fb[ScreenXY.y*w + ScreenXY.x] = TriFrag.RGB;
